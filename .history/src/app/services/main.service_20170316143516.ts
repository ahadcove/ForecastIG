...
// Will retrieve specified day: day 1 = today up to day 5
  getDay(){
    ...
    if(this.firstDay){
      ...
      // this.isFirstDay();
      this.currDay=0;
      var apiDay = this.apiObj.list[this.currDay].dt;
      var apiDayTxt = this.apiObj.list[this.currDay].dt_txt;
  ...
  findNextDay(){
    for(var iterDays = this.list[this.currDay]; iterDays<this.apiObj.list.length; iterDays++){
      ...
      console.log(testDate.getDay() != this.list[this.currDay]);
      if(testDate.getDay() != this.list[this.currDay]){
