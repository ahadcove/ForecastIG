...
fillTemps(thisDay){
  ...
  console.log(`On Day ${thisDay} at index ${this.list.indexOf(thisDay)}`);
  ...
  for(var i = 0;i<3;i++){
      ...
      console.log("time ", this.apiObj.list[nextIndex].dt)
      // currPack[0].time = this.apiObj.list[nextIndex].dt;
      currPack[0].icon = `http://openweathermap.org/img/w/`+icon+'.png';
      currPack[0].weather = this.apiObj.list[nextIndex].weather[0].main;
      currPack[0].weatherDesc = this.apiObj.list[nextIndex].weather[0].description;
      currPack[0].reg = this.apiObj.list[nextIndex].main.temp;
      currPack[0].high = this.apiObj.list[nextIndex].main.temp_max;
      currPack[0].low = this.apiObj.list[nextIndex].main.min;
      
      tempPack.push(currPack);
      nextIndex = this.calcNextIndex(thisDay, nextIndex);
  }
          return tempPack;
  }

calcNextIndex(thisDay,currIndex){
