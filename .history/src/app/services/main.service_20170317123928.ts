...
// Will retrieve specified day: day 1 = today up to day 5
  getDay(){
    ...
    //If firstDay We know we can start from the 0th position in the API Obj
    if(this.firstDay){
...
// Get's the position of the first time it finds a next day
  findNextDay(){
...
// Fill next three temperatures according to how much room we have between today and the next
fillTemps(thisDay){
  ...
  if(this.list[currIndex+1]){
      ...
      isLastIndex=true;
      nextDay = this.apiObj.list.length;
...
calcNextIndex(thisDay, nextDay, nextIndex){
  ...
  console.log(`Next ${nextIndex} Incr ${incrementIndex}`);
